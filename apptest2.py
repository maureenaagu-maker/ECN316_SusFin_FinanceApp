...
if allow_leverage:
    y = min(max(y, 0.0), 1.5)   # soft cap for cleaner presentation
else:
    y = min(max(y, 0.0), 1.0)
...
def explain_portfolio():
    ...
    explanation = (
        f"The recommended risky allocation leans most heavily toward **{higher_weight_asset}**. "
        f"Your risk aversion (γ = {gamma:.1f}) determines how much overall risk you are willing to hold, "
        f"while your ESG preference (λ = {esg_pref:.3f}) rewards portfolios with stronger sustainability characteristics. "
        f"Here, **{stronger_esg_asset}** is the stronger ESG asset, while **{higher_return_asset}** offers the higher expected return."
    )

    if y < 1:
        explanation += " Part of the portfolio remains in the risk-free asset to reduce volatility."
    elif np.isclose(y, 1.0):
        explanation += " The full portfolio is invested in risky assets."
    else:
        explanation += " Because leverage is enabled, the model recommends borrowing to increase exposure to risky assets."
...
with tab3:
    ...
    st.markdown(
        ...
        where \( \gamma \) is risk aversion and \( \lambda \) captures ESG preference strength.
