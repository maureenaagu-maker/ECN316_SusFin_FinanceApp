...
with tab2:
    ...
    ax1.scatter(
        ...
        s=170,
        marker="X",
        label="Your final recommended portfolio",
    )
    ax1.scatter(0, r_free, edgecolors="black", linewidths=0.8, s=140, marker="s", label="Risk-free asset")

    sd_line = np.linspace(0, max(risks) * 1.2, 100)
    if sd_opt_risky > 0:
        ret_line = r_free + ((ret_opt_risky - r_free) / sd_opt_risky) * sd_line
        ax1.plot(sd_line, ret_line, linestyle="--", linewidth=2, color="black", label="Capital Market Line (CML)")
    ...
    st.markdown(
        ...
        "black X = your final recommended portfolio, "
        "square = the risk-free asset, "
        "dashed line = Capital Market Line showing different mixes of the risk-free asset and risky investing."
    )
...
with tab3:
    ...
    if exclude_low_esg:
        ax2.axvline(esg_floor, linestyle="--", linewidth=2, color="black", label=f"Minimum ESG score = {esg_floor}")
